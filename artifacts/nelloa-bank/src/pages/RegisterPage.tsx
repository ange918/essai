...
const infoSchema = z.object({
  ...
  dateOfBirth: z.string().min(1, "Date de naissance requise").refine((val) => {
    if (!val) return false;
    return calcAge(val) >= 18;
...
export function RegisterPage() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith("image/")) {
      setIdDocumentError("Veuillez sélectionner une image (JPG, PNG, etc.)");
      return;
    }
    if (file.size > 5 * 1024 * 1024) {
      setIdDocumentError("L'image ne doit pas dépasser 5 Mo");
      return;
    }
    setIdDocumentError("");
  ...
  const onInfoSubmit = (data: InfoFormValues) => {
    if (!idDocument) {
      setIdDocumentError("Veuillez télécharger une pièce d'identité valide");
      return;
    }
                  ...
                  <form onSubmit={form.handleSubmit(onInfoSubmit)} className="space-y-6">
                    ...
                    <FormField control={form.control} name="dateOfBirth" render={({ field }) => (
                      <FormItem>
                        ...
                        <p className="text-xs text-muted-foreground">Vous devez avoir au moins 18 ans pour ouvrir un compte.</p>
                        <FormMessage />
                      </FormItem>
                    )} />
                    ...
                    <div>
                      <label className="block text-sm font-medium mb-2">
                        Pièce d'identité <span className="text-muted-foreground font-normal">(CNI, passeport, titre de séjour)</span>
                      </label>
                      <input
                        ref={fileInputRef}
                        type="file"
                      ...
                      {!idDocument ? (
                        <button
                          ...
                          className="w-full border-2 border-dashed border-slate-300 rounded-xl p-8 flex flex-col items-center gap-3 hover:border-primary hover:bg-primary/5 transition-all duration-200 cursor-pointer"
                        >
                        ...
                        <div className="border-2 border-green-400 bg-green-50 rounded-xl p-4 flex items-center justify-between">
                          ...
                          <button
                            type="button"
                            onClick={() => { setIdDocument(""); setIdDocumentName(""); if (fileInputRef.current) fileInputRef.current.value = ""; }}
                      ...
                      {idDocumentError && (
                        <p className="text-sm text-destructive mt-2">{idDocumentError}</p>
                      )}
                    </div>
                ...
                <div className="bg-slate-50 rounded-xl p-6 mb-8 border border-slate-200 space-y-6">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-foreground">
                        {new Date(form.getValues().dateOfBirth).toLocaleDateString('fr-FR')}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-green-700 text-sm">{idDocumentName} — Téléchargée ✓</p>
                    </div>
                  </div>
                </div>
